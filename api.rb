...
class SetupJob
  ...
  def self.perform(host, user, password, id, domain, ip, callback, user_email, user_password)
      puts "Running setup..."
      Cloudfoundry::Manager.load_config('clouds.yaml')
      paas = Cloudfoundry::Manager::Bootstrap.new(host, user, password)
      domain2 = domain.sub('api.', '') if domain.start_with?('api.')
      paas.setup(id, domain2, ip, user_email, user_password)
      puts "Setup about to end. Calling POST callback"
      require 'net/http'
      puts callback
      puts URI.unescape(callback)
      url = URI.parse(URI.unescape(callback))
      puts url
      req = Net::HTTP::Post.new(url.path)
      req.basic_auth 'portal', 'qwerty321' # TODO
      req.set_form_data({"id" => id, "endpoint" => "http://#{domain}", "key" => user_email, "secret" => user_password})
...
post '/setup' do
  ...
  Resque.enqueue(SetupJob, params['host'], params['user'], params['password'], params['id'], params['domain'], params['ip'], params['callback'], params['user_email'], params['user_password'])
  puts "Enqueued setup"
  status 202
end
...
post '/stop' do
  puts "Executing PaaS stop operation"
  Cloudfoundry::Manager.load_config('clouds.yaml')
  configs = Cloudfoundry::Manager.config['clouds'].select{ |p| p[:id] == params[:id] }
  return status 404 if configs.nil? or configs.empty?
  Resque.enqueue(StopJob, configs.first)
  puts "Ended executing PaaS stop operation"
  status 202
end
